...
class PPO_bipedal_walker_train():
    def __init__(self,
        ...
        rw = pd.DataFrame(data.local_reward)
        rw.to_csv('reward')
        rt = pd.DataFrame(data.local_return)
        rt.to_csv('return')
        ts = pd.DataFrame(data.timestep)
        ts.to_csv('timestep')
        # print(data.local_reward)
        # print(data.local_return)
...
trainer = PPO_bipedal_walker_train(
                                PATH='bipedal_env\\',
                                number_of_envs = 4,
                                reward_index = np.array([[0.33, 0.33, 0., 0.33]]),
                                data_size = 4000,
                                batch_size = 2000,
                                thresh=4.5,
                                explore = 1e-6,
